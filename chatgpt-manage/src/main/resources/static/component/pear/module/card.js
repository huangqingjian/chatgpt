layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
        ...
        if (!!url) {
            ...
            getData(url).then(function(data){
                ...
                if (!!option.data && option.data.length > 0) {
                    html = createComponent(option.elem.substring(1), option.linenum, option.data, option.hideText);
                    html += "<div id='cardpage'></div>";
                }
                else {
                    html = "<p style='text-align: center;'>没有数据</p>";
                }
                $(option.elem).html(html);
                if (option.page) {
                    laypage.render({
    ...
    function createComponent(elem,linenum,data, hideText) {
        var html = "<div class='cloud-card-component'>"
        var content = createCards(elem, linenum,data, hideText);
        var page = "";
    ...
    function createCards(elem, linenum,data, hideText) {
        var content = "<div class='layui-row layui-col-space30'>";
        for (var i = 0; i < data.length; i++) {
            content += createCard(elem, linenum,data[i],i, hideText);
        }
        content += "</div>";
        return content;
    }

    function createCard(elem, linenum, item, no, hideText) {
        var line = 12 / linenum;
        var card;
        if(hideText) {
            card = '<div id=' + item.id + ' onclick="cardTableCheckedCard(' + elem + ',this, ' + hideText + ')" class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + no + '" data-number="1"> <div class="project-list-item"> <img class="project-list-item-cover" src="' + item.image + '"> </div > </div > '
        } else {
            card = '<div id=' + item.id + ' onclick="cardTableCheckedCard(' + elem + ',this, ' + hideText+ ')" class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + no + '" data-number="1"> <div class="project-list-item"> <img class="project-list-item-cover" src="' + item.image + '"> <div class="project-list-item-body"> <h2>' + item.title + '</h2> <div class="project-list-item-text layui-text">' + item.remark + '</div> <div class="project-list-item-desc"> <span class="time">' + item.time + '</span> <div class="ew-head-list"></div> </div> </div > </div > </div > '
        }
        return card;
    }
    ...
    window.cardTableCheckedCard = function (elem,obj, hideText) {
