        ...
        $.each(option.data, function (i, item) {
            ...
            if (item.type == 1) {
                ...
                content += '<a  href="javascript:;" menu-type="' + item.type + '" menu-id="' + item.id + '" href="' + href +
                    '" ' + target + '><i class="' + item.icon + '"></i><span>' + item.name +
                    '</span></a>';
            } else if (item.type == 2) {
                content += '<a class="' + className + '" menu-type="' + item.type + '" menu-url="' + item.url + '" menu-id="' +
                    item.id +
                    '" menu-title="' + item.name + '"  href="' + href + '"  ' + target + '><i class="' + item.icon +
    ...
    function createMenuAndControl(option) {
        ...
        $.each(option.data, function (i, item) {
            ...
            if (i === option.defaultMenu) {
                controlItem = '<li pear-href="' + item.url + '" pear-title="' + item.name + '" pear-id="' + item.id +
                    '" class="layui-this layui-nav-item"><a href="#">' + item.name + '</a></li>';
                menuItem = '<ul  pear-id="' + item.id + '" lay-filter="' + option.elem +
                    '" class="layui-nav arrow layui-nav-tree pear-nav-tree">';

                controlPe += '<li class="layui-nav-item"><a class="pe-title" href="javascript:;" >' + item.name + '</a>';

                controlItemPe += '<dd  pear-href="' + item.url + '" pear-title="' + item.name + '" pear-id="' + item.id +
                    '"><a href="javascript:void(0);">' + item.name + '</a></dd>';
            } else {

                controlItem = '<li  pear-href="' + item.url + '" pear-title="' + item.name + '" pear-id="' + item.id +
                    '" class="layui-nav-item"><a href="#">' + item.name + '</a></li>';
            ...
            $.each(obj.children, function (i, note) {
                ...
                if (note.type == 1) {
                    ...
                    content += '<a ' + target + '  href="' + href + '" menu-type="' + note.type + '" menu-id="' + note.id +
                        '"><i class="' + note.icon + '"></i><span>' + note.name + '</span></a>';
                } else if (note.type == 2) {
                    // 创 建 菜 单 结 构
                    content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.url +
                        '" menu-id="' + note.id + '" menu-title="' + note.name + '" menu-icon="' + note.icon + '" href="' + href +
                        '" ><i class="' + note.icon + '"></i><span>' + note.name + '</span></a>';
                }
                // 加 载 子 项 目 录
                content += loadchild(note);
